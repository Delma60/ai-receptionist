...
const planConfig = {
  starter: { label: "Starter", class: "border-zinc-500/20 text-zinc-400 bg-zinc-500/5" },
  growth: { label: "Growth", class: "border-emerald-500/20 text-emerald-400 bg-emerald-500/10" },
  pro: { label: "Pro", class: "border-violet-500/20 text-violet-400 bg-violet-500/10" },
};
...
export default function AdminTenantDetailPage() {
  ...
  useEffect(() => {
    ...
    const unsubAgents = onSnapshot(collection(db, "tenants", tenantId, "agents"), (snapshot) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-sm text-zinc-500">
              <Link href="/admin/tenants" className="hover:text-emerald-400 transition-colors">Tenants</Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge className={cn("rounded-md px-2 py-0.5 text-[10px] uppercase font-bold", planConfig[tenant.plan]?.class)}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Minutes Used", value: `${tenant.minutesUsed || 0} / ${tenant.minutesLimit || 500}`, icon: Clock, color: "text-emerald-400" },
          { label: "Total Calls", value: tenant.totalCalls || calls.length, icon: PhoneCall, color: "text-violet-400" },
          { label: "Agents", value: agents.length, icon: Bot, color: "text-sky-400" },
          { label: "MRR", value: tenant.plan === 'pro' ? '$349.00' : tenant.plan === 'growth' ? '$149.00' : '$49.00', icon: CreditCard, color: "text-amber-400" },
        ].map((stat) => (
          <Card key={stat.label} className="border-white/[0.06] bg-zinc-900/40 backdrop-blur-sm">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList className="bg-zinc-900/60 border border-white/[0.06] p-1 h-11 backdrop-blur-sm">
          <TabsTrigger value="overview" className="data-[state=active]:bg-white/[0.08] px-6">Overview</TabsTrigger>
          <TabsTrigger value="agents" className="data-[state=active]:bg-white/[0.08] px-6">Agents</TabsTrigger>
          <TabsTrigger value="calls" className="data-[state=active]:bg-white/[0.08] px-6">Recent Calls</TabsTrigger>
        </TabsList>
          ...
          <Card className="border-white/[0.06] bg-zinc-900/80">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center py-2 border-b border-white/[0.03]">
                ...
                <span className="text-sm text-zinc-200 font-mono">{tenant.supportPhone || tenant.phoneNumber || "—"}</span>
              </div>
              <div className="flex justify-between items-start py-2">
                <span className="text-sm text-zinc-500">Business Address</span>
                <span className="text-sm text-zinc-200 max-w-[200px] text-right leading-tight">{tenant.address || "No address provided"}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-white/[0.06] bg-zinc-900/80">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center py-2 border-b border-white/[0.03]">
                ...
                <span className="text-sm text-zinc-200">{tenant.createdAt?.toDate ? tenant.createdAt.toDate().toLocaleDateString() : "Recent"}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-sm text-zinc-500">Active Subscriptions</span>
             ...
             <Table>
              <TableHeader className="bg-white/[0.02]">
                <TableRow className="border-white/[0.06] hover:bg-transparent">
                  <TableHead className="text-zinc-500 font-medium">Agent Name</TableHead>
                  <TableHead className="text-zinc-500 font-medium">Vapi ID</TableHead>
                  <TableHead className="text-zinc-500 font-medium">Status</TableHead>
                  <TableHead className="text-right text-zinc-500 font-medium pr-6">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {agents.length > 0 ? agents.map((agent) => (
                  <TableRow key={agent.id} className="border-white/[0.06] hover:bg-white/[0.01] transition-colors">
                    <TableCell className="font-medium text-zinc-200">{agent.name}</TableCell>
                    <TableCell className="text-xs font-mono text-zinc-500">{agent.vapiAgentId || "—"}</TableCell>
                    <TableCell>
                      <Badge className={cn(
                        "rounded-md px-2 py-0.5 text-[10px] uppercase font-bold",
                        agent.status === 'active' ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-zinc-800 text-zinc-500"
                      )}>
                        {agent.status}
                      </Badge>
                    </TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={4} className="h-32 text-center text-zinc-500">No agents configured for this tenant.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
             ...
             <Table>
              ...
              <TableBody>
                {calls.length > 0 ? calls.map((call) => (
                  <TableRow key={call.id} className="border-white/[0.06] hover:bg-white/[0.01] transition-colors">
                    <TableCell className="font-medium text-zinc-200">{call.caller}</TableCell>
                    <TableCell className="text-sm text-zinc-400">{call.agentName || "Agent"}</TableCell>
                    <TableCell>
                      <Badge className={cn(
                        "rounded-md px-2 py-0.5 text-[10px] uppercase font-bold",
                        outcomeStyles[call.outcome] || "bg-zinc-800 text-zinc-500"
                      )}>
                        {call.outcome}
                      </Badge>
                    </TableCell>
