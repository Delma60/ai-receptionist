  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-sm text-zinc-500">
              ...
              <Link href={`/admin/tenants/${tenantId}`} className="hover:text-emerald-400 transition-colors">Details</Link>
              <ChevronRight className="h-3 w-3" />
      ...
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold text-white tracking-tight">Edit Tenant Account</h1>
        <p className="text-sm text-zinc-500">
          Update business identity and platform restrictions for <span className="text-zinc-300 font-medium">{formData.name}</span>.
        </p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
          ...
          <CardContent className="p-6 space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-zinc-400">Company Name</label>
                <Input 
                  value={formData.name}
              ...
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-zinc-400">Account Email</label>
                <Input 
                  type="email"
        ...
        <Card className="border-white/[0.06] bg-zinc-900/80">
          ...
          <CardContent className="p-6 space-y-4">
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-zinc-400">Subscription Plan</label>
                <select 
                  value={formData.plan}
              ...
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-zinc-400">Minute Allowance</label>
                <Input 
                  type="number"
                  value={formData.minutesLimit}
                  onChange={e => setFormData({...formData, minutesLimit: parseInt(e.target.value) || 0})}
                  className="bg-white/[0.03] border-white/[0.08] focus-visible:ring-emerald-500/20"
                />
              </div>
            </div>
        ...
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 flex items-start gap-3">
          ...
          <p className="text-[12px] text-amber-200/60 leading-relaxed">
            <span className="font-bold text-amber-400">Warning:</span> Manual changes to plans or quotas bypass Stripe synchronization. Verify changes correspond to the tenant's current subscription agreement to avoid billing discrepancies.
          </p>
        </div>
